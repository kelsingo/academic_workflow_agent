...
def add_table(table_name, csv_file, conn, table_schema):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the table to add (e.g., 'advisors', 'course_2425', 'course_2526', 'students', 'maxcourse_deadline')"
    )
    parser.add_argument(
        "--csv_file",
        "--csv-file",
        ...
        type=str,
        required=True,
        help="Path to the CSV file containing the data to be added"
    )
    parser.add_argument(
        "--table_schema",
        "--table-schema",
        ...
        type=str,
        required=True,
        help="SQL schema for creating the table (e.g., 'CREATE TABLE advisors (id INTEGER PRIMARY KEY AUTOINCREMENT, advisor_name TEXT UNIQUE NOT NULL, email TEXT UNIQUE NOT NULL)')"
    )
    args = parser.parse_args()

    add_table(args.table_name, args.csv_file, conn, args.table_schema)
